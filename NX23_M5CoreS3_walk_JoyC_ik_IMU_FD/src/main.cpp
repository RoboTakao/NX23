...
{
    int x, y, w, h;
    uint32_t color = FACE_COLOR_WHITE;
...
void loop()  
{   
    float eX, eY, eZ, eX_pre = 0, eY_pre = 0, eZ_pre = 0;
    float deX, deY, deZ;
    float ieX = 0, ieY = 0, ieZ = 0;
    float uX, uY, uZ;
    ...
    if (walk_mode == 1) // Direction
    {
        eye_y = map(joyLY, 200, 0, -50, 50);
        eye_x = map(joyLX, 200, 0, -50, 50);
        
        TnX = map(joyLY, 200, 0, -18, 18);
    ...
    if (walk_mode == 3) // Face detection
    {
        float limit_angle_Y = 17;
        float limit_angle_Xu = 15;
        float limit_angle_Xd = 5;

        float Kpf = 0.35;
        float Kdf = 0.01;
        float Kif = 0.0;

        eye_x = map(xf, 0, 320, -80, 80);
        eye_y = map(yf, 0, 240, -80, 80);

        eX = - map(yf, 240, 0, -18, 18);
        eY = - map(xf, 0, 320, -20, 20);
        deX = (eX - eX_pre)/T;
        deY = (eY - eY_pre)/T;
        ieX = ieX + (eX + eX_pre)*T/2;
        ieY = ieY + (eY + eY_pre)*T/2;
        uX = Kpf*eX + Kif*ieX + Kdf*deX;
        uY = Kpf*eY + Kif*ieY + Kdf*deY;
        eX_pre = eX;
        eY_pre = eY;
        ...
        TnY = TnY + uY;
        TnZ = 0;
        if (TnX >= limit_angle_Xd) TnX = limit_angle_Xd; 
        if (TnX <= -limit_angle_Xu) TnX = -limit_angle_Xu;
        if (TnY >= limit_angle_Y) TnY = limit_angle_Y; 
        if (TnY <= -limit_angle_Y) TnY = -limit_angle_Y;
        rot_head_step(TnX, TnY, TnZ);
    } 
} 